...
FILE *fopen_check(const char *path, const char *mode, const char *file, int line) {
    ...
    if (fp == NULL) {
        fprintf(stderr, "Utils - Error: Failed to open file '%s' at %s:%d\n", path, file, line);
...
void fread_check(void *ptr, size_t size, size_t nmemb, FILE *stream, const char *file, int line) {
    ...
    if (result != nmemb) {
        if (feof(stream)) {
            fprintf(stderr, "Utils - Error: Unexpected end of file at %s:%d\n", file, line);
        } else if (ferror(stream)) {
            fprintf(stderr, "Utils - Error: File read error at %s:%d\n", file, line);
        } else {
            fprintf(stderr, "Utils - Error: Partial read at %s:%d. Expected %zu elements, read %zu\n",
                    file, line, nmemb, result);
        }
        fprintf(stderr, "Utils - Error details:\n");
        fprintf(stderr, "  File: %s\n", file);
        fprintf(stderr, "  Line: %d\n", line);
...
void fclose_check(FILE *fp, const char *file, int line) {
    if (fclose(fp) != 0) {
        fprintf(stderr, "Utils - Error: Failed to close file at %s:%d\n", file, line);
        fprintf(stderr, "Error details:\n");
        fprintf(stderr, "  File: %s\n", file);
        fprintf(stderr, "  Line: %d\n", line);
        exit(EXIT_FAILURE);
...
void fseek_check(FILE *fp, long off, int whence, const char *file, int line) {
    if (fseek(fp, off, whence) != 0) {
        fprintf(stderr, "Utils - Error: Failed to seek in file at %s:%d\n", file, line);
        fprintf(stderr, "Error details:\n");
...
void *malloc_check(size_t size, const char *file, int line) {
    ...
    if (ptr == NULL) {
        fprintf(stderr, "Utils - Error: Memory allocation failed at %s:%d\n", file, line);
        fprintf(stderr, "Error details:\n");
        fprintf(stderr, "  File: %s\n", file);
        fprintf(stderr, "  Line: %d\n", line);
...
void create_dir_if_not_exists(const char *dir) {
    ...
    if (stat(dir, &st) == -1) {
        if (mkdir(dir, 0700) == -1) {
            printf("Utils - ERROR: could not create directory: %s\n", dir);
            exit(EXIT_FAILURE);
        }
        printf("Utils - Created directory: %s\n", dir);
    }
}
