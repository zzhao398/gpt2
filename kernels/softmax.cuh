...
// req_3: one block per row, parallel reduction for max and sum.
// Must be launched with gridDim.x = N*T (one block per attention row).
__global__ void softmax_forward_kernel(float* out, float inv_temperature, const float* inp, int N, int T) {
    ...
    // parallel max reduction (causal: only up to own_pos)
